...
class Index extends \think\Controller
{
	...
	public function loginPro(){
		if(Cookie::has('name') ){
			...
			$b = $mod->check($_POST['username'],$_POST['password'],$checkbox);
			if($b){
				return $this->fetch("adminIndex");
			}else {
				echo $b;
	...
	public function zhuce(){
		$mod = model("Admin");
		$b = $mod->zhuce($_POST['username'],$_POST['password'],$_POST['target'],$_POST['QSDS']);
		if($b){
	...
	public function zhuceUser($flag,$username,$password,$phone){
		$user = model("User");
		if($flag == "zhuce"){
			$user->data([ 'username'  =>  $username,  'password' =>  md5($password),'phone' => $phone]);
			$user->save();
		}
		Temp::destroy(['phone' => $phone]);
